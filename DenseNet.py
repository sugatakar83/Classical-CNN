...
class DenseNet:
    ...
    def __init__(self, input_shape=None, dense_blocks=3, dense_layers=-1, growth_rate=12, nb_classes=None,
        ...
        self.dense_blocks = dense_blocks  # number of dense blocks to add to end
        self.dense_layers = dense_layers
        self.input_shape = input_shape
        self.growth_rate = growth_rate  # how many filters to add each layer
        self.weight_decay = weight_decay
        self.dropout_rate = dropout_rate  # dropout rate after each dense layer
        self.bottleneck = bottleneck  # 1X1 convolution layer used after dense layer to reduce feature maps
        self.compression = compression  # compressed output after transition layer
        self.nb_classes = nb_classes  # number of classification classes
    ...
    def build_model(self):
        ...
        x = Conv2D(2 * self.growth_rate, (3, 3),
                   ...
                   # l2 to deal with the multicollinearity (independent variables are highly correlated)
...
model_optimizer = tf.keras.optimizers.Adam(lr=1E-3, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
